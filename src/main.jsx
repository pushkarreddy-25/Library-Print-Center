...
function OperatorArea({ account, jobs, printers, selectedPrinterId, setSelectedPrinterId, search, setSearch, onPrint, onSignOut, notifications, notificationsOpen, setNotificationsOpen, notify }) {
  ...
  const [operatorSection, setOperatorSection] = useState('queue');
  ...
  return <div className="operator-page">
    <aside className="operator-sidebar">
      ...
      <div className="operator-nav"><button className={operatorSection === 'queue' ? 'operator-nav-active' : ''} onClick={() => { setStatusFilter('Waiting'); setOperatorSection('queue'); }}><LayoutDashboard size={17} /> Queue <b>{jobs.filter((job) => job.status === 'Waiting').length}</b></button><button className={operatorSection === 'printers' ? 'operator-nav-active' : ''} onClick={() => setOperatorSection('printers')}><Printer size={17} /> Printers <b>{onlinePrinters.length}</b></button><button className={operatorSection === 'activity' ? 'operator-nav-active' : ''} onClick={() => { setStatusFilter('Completed'); setOperatorSection('activity'); }}><Clipboard size={17} /> Activity</button></div>
      <div className="operator-footer"><span><span className={`online-dot ${onlinePrinters.length ? '' : 'offline-dot'}`} /> {onlinePrinters.length} {onlinePrinters.length === 1 ? 'printer' : 'printers'} connected</span><button onClick={() => setOperatorSection('settings')}><Settings size={17} /> Settings</button><button onClick={onSignOut}>Sign out</button></div>
    </aside>
...
function OperatorPrinters({ printers, onlinePrinters, selectedPrinterId, setSelectedPrinterId }) {
  return <section className="operator-panel"><div className="queue-toolbar"><div><h2>Printers</h2><span>{onlinePrinters.length} online of {printers.length} registered printers</span></div><Printer size={20} /></div><div className="printer-list">{printers.length ? printers.map((printer) => <button className={`printer-card ${selectedPrinterId === printer.id ? 'selected' : ''}`} key={printer.id} onClick={() => printer.status === 'online' && setSelectedPrinterId(printer.id)} disabled={printer.status !== 'online'}><span className={`online-dot ${printer.status === 'online' ? '' : 'offline-dot'}`} /><span><strong>{printer.name}</strong><small>{printer.system_name} · {printer.status}</small></span>{selectedPrinterId === printer.id && <b>Selected</b>}</button>) : <div className="operator-empty"><Printer size={22} /><strong>No printers registered</strong><span>Connect the Windows print agent to discover printers.</span></div>}</div></section>;
}

function OperatorSettings({ account, printerAgentConfigured }) {
  return <section className="operator-panel"><div className="queue-toolbar"><div><h2>Settings</h2><span>Operator account and print-agent status</span></div><Settings size={20} /></div><div className="operator-settings-grid"><div><span>Account</span><strong>{account.name}</strong></div><div><span>Role</span><strong>{account.role}</strong></div><div><span>Print agent</span><strong className={printerAgentConfigured ? 'setting-online' : 'setting-offline'}>{printerAgentConfigured ? 'Configured' : 'Not configured'}</strong></div><div><span>Printer source</span><strong>Supabase Realtime</strong></div></div></section>;
}
